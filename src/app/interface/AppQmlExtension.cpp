...
std::string AppFrame::AppQmlExtension::selectPage(const std::string &value)
{
    ...
    PageIndex pageType = meta->getPageIndex(jsParams["tableName"].asString());
    ...
    appFramework().invokeCpp(appFramework().getMeta(), "runtimeRoutine", pageType,
...
{
    appFramework().runAfter("readPLC", [value] {
        ...
        Json::Value res;
        const auto *meta = qobject_cast<AppMetaFlash *>(appFramework().getMeta());
        PageIndex pageName = meta->getPageIndex(jsParams["pageName"].asString());
        ...
        appFramework().invokeCpp(appFramework().getMeta(), "runtimeRoutine", pageName, QString::fromStdString(retJson));
    });
    return std::string("");
}
...
{
    appFramework().runAfter("writePLC", [value] {
        const auto &plc = appFramework().getPlugin<BasePLCDevice>();
        if (plc == nullptr || value.empty())
            return;
        bool ret = true;
        ...
        if (jsParams.isMember("pageName"))
        {
            ...
            const auto *meta = qobject_cast<AppMetaFlash *>(appFramework().getMeta());
            pageName = meta->getPageIndex(jsParams["pageName"].asString());
            jsParams.removeMember("funcName");
            jsParams.removeMember("pageName");
            // 修改方瓶、 圆瓶
        ...
        if (!funcName.empty())
        {
            ...
            appFramework().invokeCpp(appFramework().getMeta(), "runtimeRoutine", pageName,
                                     QString::fromStdString(retJson));
        }
    });
    return "";
}
...
std::string AppFrame::AppQmlExtension::zeroClearing(const std::string &)
{
    const auto &product = appFramework().getPlugin<BaseProduct>();
    const auto &plc = appFramework().getPlugin<BasePLCDevice>();
    if (product == nullptr || plc == nullptr)
        return "";
    bool bRet = true;
    if (product->getType() == TypeProduct::TypeCircle || product->getType() == TypeProduct::TypeImpurity)
    {
        bRet = plc->writeBitValue(0, 12992, 1, 1);
        std::this_thread::sleep_for(std::chrono::milliseconds(200));
        bRet = plc->writeBitValue(0, 12992, 1, 0);
    }
    else if (product->getType() == TypeProduct::TypeCap || product->getType() == TypeProduct::TypeLine ||
...
std::string AppFrame::AppQmlExtension::refreshMainPage(const std::string &)
{
    ...
    appFramework().runAfter("refreshMainPage", [] {
        ...
        jsMainVal["textProduceState"] = plcDev->getDeviceData(DeviceUnion::State);
        jsMainVal["textEquipmentSteps"] = plcDev->getDeviceData(DeviceUnion::Step);
...
{
    appFramework().runAfter("refreshPowerPage", [sqlFlag] {
        const auto &plcDev = appFramework().getPlugin<BasePLCDevice>();
        const auto &product = appFramework().getPlugin<BaseProduct>();
        ...
        else if (product->getType() == TypeProduct::TypeLine || product->getType() == TypeProduct::TypeCap ||
                 product->getType() == TypeProduct::TYpeBoxApp)
        {
        ...
        appFramework().invokeCpp(appFramework().getMeta(), "runtimeRoutine", PageIndex::PageElectric,
                                 QString::fromStdString(resJson));
    });
    return Utils::makeResponse(true);
}
...
{
    appFramework().runAfter("produceReset", [] {
        const auto &product = appFramework().getPlugin<BaseProduct>();
        const auto &cameraManager = appFramework().getPlugin<CameraManager>();
        const auto &cognex = appFramework().getPlugin<Cognex>();
        const auto &printerDevice = appFramework().getPlugin<PrinterDevice>();
        ...
        cognex->cleanCount();
        cameraManager->cleanCameraCount();
        ...
        else if (product->getType() == TypeProduct::TypeCircle || product->getType() == TypeProduct::TypeImpurity)
        { // 回转式
            plcDev->writeArray<uint16_t>(0, 13005, std::vector<uint16_t>(36, 0));
            plcDev->writeArray<uint16_t>(0, 13049, std::vector<uint16_t>(16, 0));
            plcDev->writeBitValue(0, 12992, 1, 1);
            plcDev->writeBitValue(0, 12992, 5, 1);
            std::this_thread::sleep_for(std::chrono::milliseconds(2000));
            plcDev->writeBitValue(0, 12992, 1, 0);
...
std::string AppFrame::AppQmlExtension::initPageData(const std::string &type)
{
    ...
    const auto *meta = qobject_cast<AppMetaFlash *>(appFramework().getMeta());
    PageIndex pageType = meta->getPageIndex(type);
    if (type == "formula")
    {
...
std::string AppFrame::AppQmlExtension::getPLCCount(const std::string &data)
{
    ...
    countJson["in"] = plc->getDeviceData(DeviceUnion::In);
    countJson["out"] = plc->getDeviceData(DeviceUnion::Out);
    countJson["camera"] = plc->getDeviceData(DeviceUnion::Ocr);
    countJson["tangleCamera"] = plc->getDeviceData(DeviceUnion::Location);
    countJson["checkCamera"] = plc->getDeviceData(DeviceUnion::LocateCheck);
    countJson["qrcode"] = plc->getDeviceData(DeviceUnion::Qrcode);
    countJson["code"] = plc->getDeviceData(DeviceUnion::Print);
    countJson["reciveCamera"] = cameraManager->getCameraCount("OCR");
    countJson["reciveTangleCamera"] = cameraManager->getCameraCount("Location");
    countJson["reciveCheckCamera"] = cameraManager->getCameraCount("LocateCheck");
    countJson["reciveCognex"] = cognex->getCount();
    return Utils::makeResponse(true, std::move(countJson), "getPLCCount");
}

std::string AppFrame::AppQmlExtension::funcTest(const std::string &data)
{
    const auto &test = appFramework().getPlugin<ExtraTest>();
    Json::Value jsParams = Utils::stringToJson(data);
    if (jsParams["project"] == "print")
    {
        if (jsParams.isMember("data"))
        {
            const auto &flag = jsParams["data"].asString();
            test->setPrintTest(flag == "1" ? true : false);
        }
    }
    else if (jsParams["project"] == "remove")
    {
        if (jsParams.isMember("data"))
        {
            const auto &typeValue = jsParams["data"].asInt();
            test->setTestRemove(static_cast<TestRemoveType>(typeValue));
        }
    }
    return "";
}

std::string AppFrame::AppQmlExtension::collectTest(const std::string &data)
{
    const auto &udp = appFramework().getPlugin<UDPCamera>();
    if (udp == nullptr)
    {
        LogInfo("udp in collectTest is null");
        return "";
    }
    udp->triggerImage();
    return "";
