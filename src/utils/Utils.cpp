...
bool Utils::removeOutdatedFiles(const std::string &currentTime, const std::string &path, int offsetDay)
{
	...
	for (auto item : files)
	{
		...
		std::chrono::system_clock::time_point timePoint = std::chrono::time_point_cast<std::chrono::system_clock::duration>(modificationTime - std::filesystem::file_time_type::clock::now() + std::chrono::system_clock::now());
